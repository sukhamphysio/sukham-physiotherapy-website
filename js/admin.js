...
document.addEventListener('DOMContentLoaded', () => {
    ...
    state.doctors.forEach(doc => {
      ...
      item.innerHTML = `
        <div class="crud-info">
          ...
          <div class="crud-details">
            <h4>${doc.name}</h4>
            <p>${doc.title} (${doc.gender}) &bull; <strong style="text-transform: capitalize; color: var(--primary-color);">${doc.category || 'medical'}</strong></p>
          </div>
        </div>
        <div class="crud-actions">
  ...
  function openDoctorModal(id) {
    ...
    document.getElementById('doctor-modal-title').textContent = doc ? 'Edit Team Member' : 'Add Team Member';
    document.getElementById('doctor-form-id').value = id || '';
    document.getElementById('doctor-form-category').value = doc ? (doc.category || 'medical') : 'medical';
  ...
  function openServiceModal(id) {
    ...
    document.getElementById('service-form-details').value = srv ? (srv.details || '') : '';
  ...
  function openBlogModal(id) {
    ...
    document.getElementById('blog-form-author').value = blog ? blog.author : 'Dr. Disha Ranade (PT)';
  ...
  function setupFormSubmissions() {
    ...
    document.getElementById('doctor-form').addEventListener('submit', (e) => {
      ...
      const category = document.getElementById('doctor-form-category').value;
      ...
      if (id) {
        ...
        const doc = state.doctors.find(d => d.id === id);
        if (doc) {
          Object.assign(doc, { name, title, gender, image: img, bio, specialties, facebook, instagram, category });
        }
      } else {
        // Add
        const newDoc = {
          id: 'd_' + Date.now(),
          name, title, gender, image: img, bio, specialties, facebook, instagram, linkedin: '#', category
        };
    ...
    document.getElementById('service-form').addEventListener('submit', (e) => {
      ...
      const details = document.getElementById('service-form-details').value.trim();

      if (id) {
        const srv = state.services.find(s => s.id === id);
        if (srv) Object.assign(srv, { title, icon, desc, details });
      } else {
        state.services.push({
          id: 's_' + Date.now(),
          title, icon, desc, details
        });
      }
